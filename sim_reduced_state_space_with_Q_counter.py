    ...
    def run(self):
        ...
        for episode in tqdm(range(self.n_episodes), ncols=100):
            ...
            for hour in range(0, self.n_hours):
                ...
                PAR = max(total_load, self.last_total_load) / average
            ...
            self.total_list.append(total_load)
        ...
        np.save(id_run + '_total_list.npy', self.total_list)
...
#         Q = np.array(Q).reshape((self.n_divisions_for_soc, self.n_hours))
    ...
    def get_final_soc_penalty(self, hour):
        ...
        if hour >= 21 and hour < 24:
            ...
            if mu >= 0.48 - (23 - hour) * self.charging_soc_addition_per_time_unit_per_ev:
                penalty = 3
            else:
                penalty = -3
