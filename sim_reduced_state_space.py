...
forecast_flag = False
n_percent_honesty = ['0.33', '0.66', '0.99']
        ...
        for episode in tqdm(range(self.n_episodes), ncols=100):
            ...
            for hour in range(0, self.n_hours):
                ...
                for division in range(0, self.n_divisions_for_soc):
                    ...
                    Q = self.Q[division][hour][int(float(percent_honest)/0.33 - 1)]
                ...
                for division in range(0, self.n_divisions_for_soc):
                    ...
                    if next_hour <= 23:
                        delta = (reward 
                                 + self.gamma * np.max(self.Q[division][next_hour][int(float(next_percent_honest)/0.33-1)])
                                 - self.Q[division][hour][int(float(percent_honest)/0.33-1)][div_to_action_dict[division][0]])
                        self.Q[division][hour][int(float(percent_honest)/0.33-1)][div_to_action_dict[division][0]] += self.alpha * delta
                    else:
                        delta = reward - self.Q[division][hour][int(float(percent_honest)//0.33-1)][div_to_action_dict[division][0]]
                        self.Q[division][hour][int(float(percent_honest)/0.33-1)][div_to_action_dict[division][0]] += self.alpha * delta
...
#         Q = np.array(Q).reshape((self.n_divisions_for_soc, self.n_hours))
    ...
    def get_final_soc_penalty(self, hour):
        ...
        if hour >= 21 and hour < 25:
            mu = np.mean(self.soc_of_evs)
            sigma = np.std(self.soc_of_evs)
            if mu >= 0.5 - (24 - hour) * self.charging_soc_addition_per_time_unit_per_ev:
                penalty = 3
            else:
                penalty = -3
